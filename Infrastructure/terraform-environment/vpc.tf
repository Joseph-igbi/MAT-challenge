...
#attach the route to the private subnet. Main difference between private and public subnets is that the route table points to the nat gateway
resource "aws_route_table_association" "rt-priv-assign" {
  subnet_id      = aws_subnet.private_subnet.id
  route_table_id = aws_route_table.route-table-priv.id
}

resource "aws_security_group" "sgbh" {
  name = "sg_bastion"
...
resource "aws_security_group" "sgph" {
  name = "sg_private"
